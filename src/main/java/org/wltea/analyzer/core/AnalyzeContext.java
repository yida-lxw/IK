...
import static org.wltea.analyzer.core.Lexeme.TYPE_ARABIC_COUNT;
import static org.wltea.analyzer.core.Lexeme.TYPE_ARABIC_EN_UNIT;
import static org.wltea.analyzer.core.Lexeme.TYPE_CNUM_DENARY;
import static org.wltea.analyzer.core.Lexeme.TYPE_CNWORD;
...
class AnalyzeContext {
	...
	//英文常见单位
	private Set<String> english_unit = new HashSet<String>();
	...
	private void outputSingleCJK(int index) {
		if (CharacterUtil.CHAR_CHINESE == this.charTypes[index]) {
			// 检查是否应该输出单个汉字
			if (shouldOutputSingleChar(index)) {
	 ...
	 * 仅当该汉字在词典中存在时才输出
	 *
	 * @param index 汉字在缓冲区中的位置索引
	 * @return 是否应该输出
	 */
	private boolean shouldOutputSingleChar(int index) {
		// 获取汉字字符
		String singleChar = String.valueOf(this.segmentBuff[index]);
		// 检查是否在主词典中
		boolean inMainDict = Dictionary.getSingleton().isMatchInMainDict(singleChar);
		// 检查是否在中文单位词典中
		boolean inQuantifierDict = Dictionary.getSingleton().isMatchInQuantifierDict(singleChar);
		// 检查是否在英文单位词典中（可能包含一些常用单字）
		boolean inEnglishUnit = english_unit.contains(singleChar);
		return inMainDict || inQuantifierDict || inEnglishUnit;
	}
    ...
    /*public ReLexeme getReLexemeText(Lexeme result) {
        int type = result.getLexemeType();
        if ((type != TYPE_CNWORD) && (type != Lexeme.TYPE_OTHER_CJK) &&
                (type != Lexeme.TYPE_CNCHAR) && (type == this.reLexeme.getLexemeType()) &&
                (result.getBeginPosition() - this.reLexeme.getBegin() == this.reLexeme.getLength())) {
            this.reLexeme.setLength(this.reLexeme.getLength() + result.getLength());

            String reLexemeText = this.reLexeme.populateLexemeText(this.segmentBuff,this.reLexeme);
            this.reLexeme.setLexemeText(reLexemeText +
                    String.valueOf(this.segmentBuff, result.getBegin(), result.getLength()));
            *//*this.reLexeme.setLexemeText(this.reLexeme.getLexemeText() +
                    String.valueOf(this.segmentBuff, result.getBegin(), result.getLength()));*//*
        } else {
            this.reLexeme.setLength(result.getLength());
            this.reLexeme.setBegin(result.getBegin());
            this.reLexeme.setLexemeText(String.valueOf(this.segmentBuff, result.getBegin(), result.getLength()));
            this.reLexeme.setLexemeType(result.getLexemeType());
        }
        return this.reLexeme;
    }*/
	 ...
	 * @param result
	 */
	public void combinWord(Lexeme result) {
		//如果是细粒度切分，就不需要合并了
		...
		do {
			...
			if (Lexeme.TYPE_ARABIC != result.getLexemeType() &&
					Lexeme.TYPE_CNUM != result.getLexemeType()) {
				break;
			}
			//当前词是阿拉伯数字
			if (Lexeme.TYPE_ARABIC == result.getLexemeType()) {
				...
				if (Lexeme.TYPE_DENARY == nextLexeme.getLexemeType()) {
					//如果是智能分词
					if (this.cfg.useSmart()) {
						//合并阿拉伯数字+中文十进制,比如：2百万，8千
						appendOk = result.append(nextLexeme, TYPE_CNWORD);
					}
				}
				//中文量词
					...
					//如果是智能分词
					if (this.cfg.useSmart()) {
						//合并阿拉伯数字+中文量词，比如：2天  24小时
						appendOk = result.append(nextLexeme, TYPE_ARABIC_COUNT);
					}
				}
				//英文单位
				else if (Lexeme.TYPE_EN_UNIT == nextLexeme.getLexemeType()) {
					//如果是智能分词
					if (this.cfg.useSmart()) {
						//合并阿拉伯数字+英文单位，比如：80KG  160cm  3600s
						appendOk = result.append(nextLexeme, TYPE_ARABIC_EN_UNIT);
					}
				} else if (Lexeme.TYPE_ARABIC == nextLexeme.getLexemeType()) {
					//合并数字+数字
			...
			//如果当前词是中文数字
				...
				if (Lexeme.TYPE_DENARY == nextLexeme.getLexemeType()) {
					//如果是智能分词
					if (this.cfg.useSmart()) {
						//合并中文数字+中文十进制,比如：三百万，八千
						appendOk = result.append(nextLexeme, TYPE_CNUM_DENARY);
					}
				}
				//中文量词
				else if (Lexeme.TYPE_COUNT == nextLexeme.getLexemeType()) {
	...
	//获取上一个匹配词
	public QuickSortSet.Cell getPrevLexeme() {
		QuickSortSet quickSortSet = this.getOrgLexemes();
		if (null == quickSortSet) {
			return null;
		}
		QuickSortSet.Cell tail = quickSortSet.getTail();
		if (null == tail) {
			return null;
		}
		return tail.getPrev();
	...
	//获取上一个匹配词的字符串
	public String getLexemeText(QuickSortSet.Cell cell) {
		if (null == cell) {
			return "";
		}
		return String.valueOf(this.getSegmentBuff(), cell.getLexemeBegin(), cell.getLexemeLength());
	}
}
